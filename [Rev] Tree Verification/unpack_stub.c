...
unsigned char secret_key[] = "dLkptn2}$bjVN6k)w{'GRX58u(P@61zh";
...
int _start(void) { //Entrypoint for the program
    ...
    int key_len = sizeof(secret_key) - 1;

    for(DWORD i = 0; i < sections[p_NT_HDR->FileHeader.NumberOfSections - 1].SizeOfRawData; ++i) {
        unsigned char key_byte = secret_key[i % key_len];
        unsigned char dynamic_val = i & 0xFF;
        packed_PE[i] = (packed_PE[i] ^ key_byte) - dynamic_val;
    }
